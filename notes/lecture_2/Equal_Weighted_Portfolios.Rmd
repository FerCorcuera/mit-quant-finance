...
ggplot(df_port_all, aes(x=date,y=value,col=portfolio)) + geom_line()+
  ggtitle(paste(c("Value $1000 Invested 2022-01-01\n",
...
ggplot(df_all, aes(x=date,y=Value, col=symbol))+
  geom_line() +
  theme(legend.position = "bottom")+
  ggtitle("Value of $250 Invested on 2022-01-01")+
  scale_y_continuous(labels= scales::dollar_format())
...
```{r,echo=TRUE}
# we compute the new weights at 2020603
library(dplyr)
...
```{r,echo=TRUE}
ggplot(df_weights, aes(x = date, y = weight, color = symbol)) +
  geom_line() +
  theme(legend.position = "bottom") +
  ggtitle("Portfolio Weights Over Time")

```


```{r,echo=TRUE}
df_portfolio <- df_weights %>%
  group_by(date) %>%
  summarise(portfolio_value = sum(Value)) %>%
  arrange(date)

# beginning
df_portfolio %>% slice(1)

# end
df_portfolio %>% slice(n())
...
```{r,echo=TRUE}
last_date <- df_weights %>%
  group_by(symbol) %>%
  summarise(last_date = max(date)) %>%
  summarise(common_last_date = min(last_date)) %>%
  pull(common_last_date)
# withotu rebalancing, GOOOGLE sotck dominated the portfolio
df_weights %>%
...
```{r,echo=TRUE}
initial <- df_portfolio %>% slice(1) %>% pull(portfolio_value)
final   <- df_portfolio %>% slice(n()) %>% pull(portfolio_value)

# the final return of the portfolio after two years was 50%!
return <- (final / initial) - 1
return
```

```{r,echo=TRUE}
ggplot(df_portfolio, aes(x = date, y = portfolio_value)) +
  geom_line() +
